...
class ApiMDC{
  ...
  async story_mark_as_closed(email, story_id){

      const url = this.base_url+"story_mark_as_closed";
      const dat = {
                 email: email,
                 story_id: story_id,
                 server_key: this.key
...
class Chat extends React.Component {
  ...
  setStoryCanceled(){
    ...
    if (window.confirm('Você tem certeza de que deseja encerrar essa história? Esta ação removerá essa história do processo de geração e é irreversível.')){
      dis.mapi.story_mark_as_closed(dis.state.email, dis.state.story.idd).then(function(resp){
        if(resp.status===200){
          if(resp.data.status==="success"){
            dis.requestStory()
          }else{
  ...
  setStoryCompleted(){
    ...
    if (window.confirm('Você tem certeza de que deseja marcar essa história como completa? Esta ação ira fechar o processo de geração de diálogo para essa história.')){
      dis.mapi.story_mark_as_complete(dis.state.email, dis.state.story.idd).then(function(resp){
        if(resp.status===200){
          if(resp.data.status==="success"){
            dis.requestStory()
          }else{
